...
class TeacherSpecificityScorer(BasedLocalRewardScorer):
    ...
    def reward(self, conversations, master_history=None, last_action=None, agent_dictionary=None):
        ...
        if master_history and last_action:
            master_conv = master_history.dialogues
            if master_conv[0].reward.get('specificity', None) is None:
                return None, None
...
class TeacherNoveltyScorer(BasedLocalRewardScorer):

    def reward(self, conversations, master_history=None, last_action=None, agent_dictionary=None):
        master_conv = None
        if master_history and last_action:
            master_conv = master_history.dialogues
            if master_conv[0].reward.get('novelty', None) is None:
                return None, None
            master_rewards = [d.reward['novelty'] for d in master_conv]
        diverged_rewards = []
        for conv in conversations:
            conv_reward = []
            for d in conv:
                conv_reward.append(d.reward['novelty'])
            diverged_rewards.append(conv_reward)
        if master_conv:
            return master_rewards, diverged_rewards
        else:
...
class WeightedCoverageScorer(BasedGlobalRewardScorer):
    ...
    def reward(self, conversations, master_history=None, last_action=None, agent_dictionary=None):
        ...
        if master_history and last_action:
            ...
            # conversations[0][-1].cache = master_history.get_cache(last_action)
            conversations = [master_conv] + conversations
            tokens_weights = master_history.context_token_weights
        if not tokens_weights:
            raise Exception("No weights provided in history for context tokens")
        num_convs = len(conversations)
        context_tok_idx = agent_dictionary.txt2vec(master_history.context)
        total_toks = len(context_tok_idx)
        assert len(context_tok_idx) == len(tokens_weights)
        diverged_rewards = []
        master_rewards = None
        master_cache = {}
        for i in range(num_convs):
            ...
            for j in range(len(conv)):
                if master_rewards and not conv[j].generated:
                    ...
                    if conv[j].answer == "CANNOTANSWER":
                        ...
                        new_capture_count = interviewer_coverage[j].difference(prev_coverage)
                        if new_capture_count:
                            weights = [tokens_weights[i] for i in new_capture_count]
                            weighted_sum = sum(weights)
                            dialogue_reward.append(weighted_sum)
                        else:
                            dialogue_reward.append(0)
            if master_conv and i==0:
                master_rewards = dialogue_reward
...
REWARD_MAP = {'reward_question': QuestionTokensScorer, 'reward_you': YouScorer,
              ...
              'reward_novelty': TeacherNoveltyScorer}
